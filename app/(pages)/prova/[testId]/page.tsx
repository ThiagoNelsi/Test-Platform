...
import { AlertCircle, ArrowLeft, CheckCircle, Clock, Eye, EyeClosed, EyeOff, Save, X } from "lucide-react"
...
export default function Test() {
  ...
  const totalQuestions = (examData?.submission.sections as Array<any>)?.reduce((acc: number, section: any) => acc + section.questions.length, 0) ?? 0
            ...
            <div className="flex items-center">
              {
                isSubmitting !== "submitted" && (
                  <>
                    <div className={`text-sm flex items-center gap-2 ${isTimeAlmostUp ? "text-red-500" : ""}`}>
                      Tempo restante:
                      <span className="w-[7ch]">
                        {showTimer ? formatTime(timeRemaining) : <Skeleton className="animate-none bg-neutral-300 w-[7ch] h-2" />}
                      </span>
                    </div>
                    {
          ...
          <div className="flex gap-2 md:items-end">
            {examData.test.dueDate && (
              <Badge variant="outline" className="font-normal">
                Entregar até: {examData.test.dueDate?.toLocaleDateString() + " às " + examData.test.dueDate?.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
              </Badge>
            )}
            <Badge className="bg-verdigris-400 hover:bg-verdigris-400">Valor: {examData.test.value} pontos</Badge>
          </div>
        </div>
      </div>
      {/* Header */}
          ...
          <div className="flex gap-3">
            ...
            }
            {
              isSubmitting === "submitting" ? (
                ...
                <Button className="gap-2 bg-verdigris-400 hover:bg-verdigris-300" onClick={() => setShowSubmitConfirmation(true)}>
      ...
      <AlertDialog open={showSubmitConfirmation} onOpenChange={setShowSubmitConfirmation}>
        <AlertDialogContent>
            ...
            <AlertDialogDescription>
              {answeredQuestions < totalQuestions && (
                <span className="mb-4 flex items-start gap-3 p-2 bg-amber-50 dark:bg-amber-950 text-amber-800 dark:text-amber-200 rounded-md">
                  ...
                  <span>
                    Atenção: Você ainda não respondeu todas as questões. Tem certeza que deseja entregar a prova agora?
                  </span>
                </span>
              )}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleSubmitExam}>Confirmar Entrega</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>

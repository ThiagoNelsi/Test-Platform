...
const SuccessMessage = ({ createdClassroom }: { createdClassroom: Classroom }) => {
    ...
    return (
        <div className="flex flex-col gap-2 my-4 p-4 bg-green-100 text-green-800 rounded-md">
            ...
            <p>Código: <strong>{createdClassroom.code}</strong></p>
            <Button 
...
export default function NewClassroomModal({ userId }: { userId: number }) {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>, action: (formData: FormData) => Promise<any>) {
            ...
            <DialogContent>
                ...
                <Tabs>
                    <TabsList className="w-full justify-between mb-4">
                        <TabsTrigger className="w-full" value="join">Entrar em uma turma</TabsTrigger>
                        <TabsTrigger className="w-full" value="create">Criar uma turma</TabsTrigger>
                    </TabsList>
                    <TabsContent value="join">
                        <form onSubmit={(e) => handleSubmit(e, joinClassroom)} className="flex gap-2">
                    ...
                    <TabsContent value="create">
                        {createdClassroom ? <SuccessMessage createdClassroom={createdClassroom} /> :
                        <form onSubmit={(e) => handleSubmit(e, createClassroom)}>
                            ...
                            <Button 
                                ...
                                disabled={loading}
                            >
                                {loading ? <Loader className="w-4 h-4 animate-spin" /> : "Criar"}
                            </Button>
                        </form>}
                    </TabsContent>
                </Tabs>

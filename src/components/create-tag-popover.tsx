...
import { useCreateTagMutation } from "@/src/hooks/use-api-queries";
...
export const CreateTagPopover = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await createTagMutation.mutateAsync(tag);
      inputRef.current.value = "";
      setSelectedColor(0);
      successToast("Tag criada com sucesso");
      onCreateTag(res.tag);
    } catch (error) {
      errorToast(getApiErrorMessage(error, "Não foi possível criar a tag"));
    }
  };

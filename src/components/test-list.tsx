...
import type { TestSummaryEntity } from "@/lib/test-service";
  ...
  const handlePublish = async (test: Test) => {
    ...
    try {
      await publishMutation.mutateAsync({
        testId: test.id,
        classroomIds: [test.classroom.id],
      });
      successToast("Prova publicada com sucesso");
    } catch (error) {
      errorToast(getApiErrorMessage(error, "Não foi possível publicar a prova"));
    }
  };
  ...
  const handleDelete = async (test: Test) => {
    try {
      await deleteMutation.mutateAsync({
        testId: test.id,
        softDelete: test.status !== "draft",
      });
      successToast("Prova apagada com sucesso");
    } catch (error) {
      errorToast(getApiErrorMessage(error, "Não foi possível apagar a prova"));
    }
  };
